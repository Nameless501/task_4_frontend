...
function Sign() {
    ...
    const { inputsValue, errorMessages, formIsValid, handleChange } = useFormStateAndValidation();

    const { users, status, statusCode } = useSelector((state) => state.authentication);
    ...
    async function handleSubmit(evt) {
        evt.preventDefault();
        dispatch(handleAuthentication({ payload: inputsValue, path: signFormConfig.api[location.pathname]}))
